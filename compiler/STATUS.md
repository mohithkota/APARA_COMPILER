...
| Register spilling (>28 live vars) | Not started | RuntimeError raised |
...
|---|---------|--------|---------|
| 1 | **Register spilling** (>28 live vars) | Medium | None — next |
| 2 | **Float arithmetic** (+,-,*,/) | Low | ISA `$fadd/$fsub/$fmul/$fdiv` needed |
| 3 | **Variadic functions** (printf) | High | ABI design |
| 4 | **Function pointers** | Medium | None |
| 5 | **String literals** (full) | Low | needs printf first |
| 6 | **Sub-word LD/ST** ($i32/$i16/$i8) | Low | **Hardware engine bug** |

**Overall compiler completeness: ~85% of a basic C compiler**
...
│   ├── codegen.py         ← IR → APARA mcode  (v5: 28-reg dynamic allocator)
