...
K=[("1-D 3-tap",            tap(3),                       True),
   ...
   ("2-D 3-point row",       'long long f(){vi8_t in[320],out[320];int i,j;for(i=0;i<8;i++)for(j=0;j<28;j++)out[i*32+j]=in[i*32+j]+in[i*32+j+1]+in[i*32+j+2];return out[0];}',  True),
   ("2-D 3x3 stencil",       'long long f(){vi8_t in[320],out[320];int i,j;for(i=0;i<6;i++)for(j=0;j<28;j++)out[i*32+j]=in[i*32+j]+in[i*32+j+1]+in[i*32+j+2]+in[(i+1)*32+j]+in[(i+1)*32+j+1]+in[(i+1)*32+j+2];return out[0];}', True),
   ("2-D 3-point weighted",  'long long f(){vi8_t in[320],out[320];int i,j;int a=1,b=2,c=3;for(i=0;i<8;i++)for(j=0;j<28;j++)out[i*32+j]=a*in[i*32+j]+b*in[i*32+j+1]+c*in[i*32+j+2];return out[0];}',  True),
   ("2-D 3-point vi16",      'long long f(){vi16_t in[320],out[320];int i,j;for(i=0;i<8;i++)for(j=0;j<28;j++)out[i*32+j]=in[i*32+j]+in[i*32+j+1]+in[i*32+j+2];return out[0];}', True),
   ("2-D 3-point remainder", 'long long f(){vi8_t in[320],out[320];int i,j;for(i=0;i<8;i++)for(j=0;j<20;j++)out[i*32+j]=in[i*32+j]+in[i*32+j+1]+in[i*32+j+2];return out[0];}',True),
   ("REJECT 2-D IV start!=0",'long long f(){vi8_t in[320],out[320];int i,j;for(i=1;i<6;i++)for(j=1;j<28;j++)out[i*32+j]=in[i*32+j]+in[i*32+j+1]+in[i*32+j-1];return out[0];}', False),
